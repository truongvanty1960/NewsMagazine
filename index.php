    ...
    <div class="content">
      <div id="featured_slide">
        <ul id="featurednews">
          ...
          $sql = "SELECT * FROM test1111 where hot = 1 ORDER  BY pub_date_time DESC Limit 0,5";
          ...
          if ($num_rows > 0) {
            while ($data = mysqli_fetch_assoc($query)) {
              echo "<li>";
              echo "<img class=\"post-image\" src=". $data['image'] ." />";
              echo "<div class=\"panel-overlay\">";
              echo "<p><strong><a href=\"#\">". $data['title'] ."</a></strong></p><br>";
              $img = str_replace('<img src="'.$data['image'].'?w=100" />',"",$data['description']);
              $img = str_replace('<img src="'.$data['image'].'?w=220" />',"",$img);
              echo $img;
              echo "<td><a href='full-width.php?id=" . $data['id'] . "'>
                                 Continue Reading &raquo;</a></td>";
//              echo "<a href='update.php?id=" . $data['idupload'] .'">Continue Reading &raquo;</a></p>";
              echo "</div>";
              echo "</li>";
            }
          }
...
<!--          </li>-->
    ...
    <div class="column">
      <ul class="latestnews">
        <?php
        require_once("connection.php");
        $sql = "SELECT * FROM test1111 ORDER  BY pub_date_time DESC Limit 0,3";
        //    $query = mysql_query($sql);
        $query = mysqli_query($conn, $sql);
        $num_rows = mysqli_num_rows($query);
        //$total = mysqli($query);
        if ($num_rows > 0) {
          while ($data = mysqli_fetch_assoc($query)) {
            echo "<li>";
            echo "<img class=\"post-image\" src=". $data['image'] ." />";
            echo "<p><strong><a href='full-width.php?id=" . $data['id'] . "'>". $data['title'] ."</a></strong></p><br>";
            $img = str_replace('<img src="'.$data['image'].'?w=100" />',"",$data['description']);
            $img = str_replace('<img src="'.$data['image'].'?w=220" />',"",$img);
            echo $img;
            echo "</li>";
...
<div class="wrapper">
  ...
  <div id="hpage_cats">
    <div class="fl_left">
      ...
      <?php
      require_once("connection.php");
      $sql = "SELECT * FROM test1111 where id_category = '9'";
      //    $query = mysql_query($sql);
      $query = mysqli_query($conn, $sql);
      $num_rows = mysqli_num_rows($query);
      //$total = mysqli($query);
      if ($num_rows > 0) {
        while ($data = mysqli_fetch_assoc($query)) {
          ...
          echo "<img class=\"post-image\" src=". $data['image'] ." />";
          echo "<p><strong><a href='full-width.php?id=" . $data['id'] . "'>". $data['title'] ."</a></strong></p>";
          $img = str_replace('<img src="'.$data['image'].'?w=100" />',"",$data['description']);
          $img = str_replace('<img src="'.$data['image'].'?w=220" />',"",$img);
          echo $img;
          echo "</div>";
...
<!--      <p>This is a W3C standards compliant free website template from <a href="http://www.os-templates.com/">OS Templates</a>. For more CSS templates visit <a href="http://www.os-templates.com/">Free Website Templates</a>. Condimentumfelis et amet tellent quisquet a leo lacus nec augue accumsan sagittislaorem dolor sum at urna.</p>-->
    ...
    <div class="fl_right">
      ...
      <?php
      require_once("connection.php");
      $sql = "SELECT * FROM test1111 where id_category = '4'";
      //    $query = mysql_query($sql);
      $query = mysqli_query($conn, $sql);
      $num_rows = mysqli_num_rows($query);
      //$total = mysqli($query);
...
<!--      <p>This template is distributed using a <a href="http://www.os-templates.com/template-terms">Website Template Licence</a>, which allows you to use and modify the template for both personal and commercial use when you keep the provided credit links in the footer.</p>-->
    ...
    <div class="fl_left">
      ...
      <?php
      require_once("connection.php");
      $sql = "SELECT * FROM test1111 where id_category = '10'";
      //    $query = mysql_query($sql);
      $query = mysqli_query($conn, $sql);
      $num_rows = mysqli_num_rows($query);
      //$total = mysqli($query);
...
<!--      <p>Morbitincidunt maurisque eros molest nunc anteget sed vel lacus mus semper. Anterdumnullam interdum eros dui urna consequam ac nisl nullam ligula vestassa. Condimentumfelis et amet tellent quisquet a leo lacus nec augue accumsan sagittislaorem dolor sum at urna.</p>-->
    ...
    <div class="fl_right">
      ...
      <?php
      require_once("connection.php");
      $sql = "SELECT * FROM test1111 where id_category = '5'";
      //    $query = mysql_query($sql);
      $query = mysqli_query($conn, $sql);
      $num_rows = mysqli_num_rows($query);
      //$total = mysqli($query);
